...
class UserModel:
    ...
    def save_to_db(user):
        try:
            __dbmanager__.update_data(user['email'], user)
        except Exception as e:
            logging.error(f"Error saving user to database: {str(e)}", exc_info=True)
            raise Exception('Error saving user to database')
    
    @staticmethod
    def update_verification_code(email, verification_code, expiration_time):
        try:
            __dbmanager__.update_verification_code(email, verification_code, expiration_time)
        except Exception as e:
